...
启动时会改写 Spark 分层 `rope_parameters`：保留 `full_attention` / `sliding_attention`，并加上 dict 哨兵 `rope_theta`，避免 transformers 5.x 注入 float 后报 `'float' object has no attribute 'get'`。每次启动会清空 `logs/vllm.log`。
